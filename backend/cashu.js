...
async function getInfo() {
  return reqJSON(`${mintUrl()}/v1/info`);
}

async function getKeys() {
  return reqJSON(`${mintUrl()}/v1/keys`);
}

async function mintQuoteBolt11({ amount }) {
  return reqJSON(`${mintUrl()}/v1/mint/quote/bolt11`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: { amount, unit: mintUnit() }
  });
}

async function mintBolt11(payload) {
  // Forward payload as-is to support mints requiring { quote, outputs }
  return reqJSON(`${mintUrl()}/v1/mint/bolt11`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: payload
  });
}

async function meltQuoteBolt11({ invoice, request }) {
  // Some mints expect field name 'request' (bolt11). Accept 'invoice' from client and map to 'request'.
  const bolt11 = request || invoice;
  return reqJSON(`${mintUrl()}/v1/melt/quote/bolt11`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: { request: bolt11, unit: mintUnit() }
  });
}

async function meltBolt11(payload) {
  return reqJSON(`${mintUrl()}/v1/melt/bolt11`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: payload
  });
}
...
async function checkMintQuote({ quote }) {
  ...
  try {
    return await reqJSON(`${mintUrl()}/v1/mint/quote/bolt11/${encodeURIComponent(quote)}`);
  } catch (e) {
    // Fallback to query-style some mints use: /v1/mint/quote/bolt11?quote=...&unit=sat
    if (e && (e.status === 404 || e.status === 405)) {
      try {
        return await reqJSON(`${mintUrl()}/v1/mint/quote/bolt11?quote=${encodeURIComponent(quote)}&unit=${encodeURIComponent(mintUnit())}`);
      } catch (e2) {
        // Try without unit as a last resort
        return await reqJSON(`${mintUrl()}/v1/mint/quote/bolt11?quote=${encodeURIComponent(quote)}`);
      }
    }
    throw e;
  }
...
async function swap({ inputs, outputs }) {
  return reqJSON(`${mintUrl()}/v1/swap`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: { inputs, outputs }
  });
}
